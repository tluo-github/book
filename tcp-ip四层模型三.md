...
> 可以看到当我们在浏览器中输入网址,网页通过HTTP协议回到浏览器这个并加载完端口的整个过程。我们说过HTT是应用层协议基于TCP传输层
> 协议。所以这里是先创建了TCP链接。也就是3次握手后才有HTTP。说明HTTP确实是使用TCP建立的。也侧面反映各层做各层的事，大大的方便了上一层，下篇文章中我们就写写HTTP协议。就会发现它就只需要关注它的业务需求，TCP不用管IP,MAC这些。HTTP也就会不用管网络传输。

* 实战TCP创建三次握手

    1. 我们能看到TCP创建经过了3次握手，第一次我本地192.168.11.200:12964向120.55.85.46:80发出了一个SYN\(其他参数也很重要暂时忽悠，只与为什么我输入的是域名这里确是IP这个是因为DNS，以后解释\)
    2. 第二个TCP包是服务器\(120.55.85.46：80\)向我本地\(192.168.11.200:12964\)发送了SYN,ACK
    3. 第三个TCP是我本地\(192.168.11.200:12964\)向服务器\(120.55.85.46：80\)发送ACK，然后TCP完成创建,HTTP开始工作

* 实战TCP断开四次握手

    1. 在第二个红框中最后4条(这里框多了2条)服务端数据发送完后向我本地发送了FIN,ACK
    2. 然后下一条我本地回复了ACK
    3. 紧接着我本地又回复了一个FIN,ACK
    4. 最后服务返回了ACK

可以看到，我们实战跟理论还是相结合的，所以我们在优化站点及安全时地基往往是在这些地方
